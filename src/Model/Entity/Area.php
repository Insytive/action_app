<?php
declare(strict_types=1);

namespace App\Model\Entity;

use Cake\ORM\Entity;

/**
 * Area Entity
 *
 * @property int $id
 * @property string $name
 * @property int $municipality_id
 *
 * @property \App\Model\Entity\Municipality $municipality
 * @property \App\Model\Entity\Branch[] $branches
 * @property \App\Model\Entity\Ward[] $wards
 */
class Area extends Entity
{
    /**
     * Fields that can be mass assigned using newEntity() or patchEntity().
     *
     * Note that when '*' is set to true, this allows all unspecified fields to
     * be mass assigned. For security purposes, it is advised to set '*' to false
     * (or remove it), and explicitly make individual fields accessible as needed.
     *
     * @var array
     */
    protected $_accessible = [
        'name' => true,
        'municipality_id' => true,
        'municipality' => true,
        'branches' => true,
        'wards' => true,
    ];


    protected function _getAreaMunicipality(){
        $area = ( !empty($this->name) ) ? $this->name : 'Unknown Area';
        $municipality = $this->has('municipality') ? $this->municipality->name : 'Unknown Municipality';

        return $area .' - '. $municipality;
    }
}
